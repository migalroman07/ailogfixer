...
def view_resolved_log(log: Incident):
    ...
    print(f"Original ussue: {log.raw_log.strip()}")
    print("========== Script that helped ==========")
    print(log.ai_summary.strip() if log.ai_summary else "Script is not aviable.")
    print("\n===================================================")
    ...
    while True:
        ...
        try:
            if log.executed:
                ...
                print("===========================================================\n")
                print("The script has already been executed earlier.")
            ...
            if not clean_commands or clean_commands == "MANUAL_INTERVENTION_REQUIRED":
                print("[-] AI could not generate a script.")
                log.status = (
            ...
            if q.confirm("Run it?").ask():
                ...
                if action == "success":
                    log.status = "resolved"
                    log.executed = False  # Очищаем флаг для порядка
                    db.commit()
                    print("\nIncident resolved successfully.")
                    break
                elif action == "error":
